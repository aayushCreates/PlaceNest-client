...
const OtherJobs = () => {
  ...
  const [openScrapeModal, setOpenScrapeModal] = useState(false);
  ...
  const [scrapeFormData, setScrapeFormData] = useState({
  ...
  const handleScrapeChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
  ) => {
    const { name, value } = e.target;
    setScrapeFormData({
      ...scrapeFormData,
      [name]: name === "maxJobs" || name === "jobPostedDays" ? Number(value) : value,
    });
  };
  ...
  const handleStartScrape = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_OTHERJOBS_API_URL}/job-board/scrape/jobs`,
        scrapeFormData,
        {
      ...
      toast.error("Error during job scraping");
    } finally {
      setIsScraping(false);
    }
  };
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-8 lg:p-10">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">AI Job Scraper</h2>
              <p className="text-sm text-slate-500 font-medium">
                Directly scrape jobs from various platforms
              </p>
            </div>
            <button
              onClick={() => setOpenScrapeModal(false)}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  name="platform"
                  value={scrapeFormData.platform}
                  onChange={handleScrapeChange}
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3.5 outline-none focus:bg-white focus:border-blue-500 transition-all font-medium text-slate-800 appearance-none"
                >
            ...
            {scrapeFormData.platform === "linkedin" ? (
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2 ml-1">
                  Search Query
                </label>
                <input
                  type="text"
                  name="inputQuery"
                  value={scrapeFormData.inputQuery}
                  onChange={handleScrapeChange}
                  placeholder="e.g. SDE Intern hiring"
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3.5 outline-none focus:bg-white focus:border-blue-500 transition-all font-medium text-slate-800"
                />
              </div>
            ) : (
            ...
            {scrapeFormData.platform === "unstop" && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-blue-50/50 rounded-2xl border border-blue-100">
                <div>
                  ...
                  <select
                    name="jobType"
                    value={scrapeFormData.jobType}
                    onChange={handleScrapeChange}
                    className="w-full bg-white border border-blue-100 rounded-xl px-2 py-2 text-xs font-bold text-slate-700 outline-none"
                  >
                ...
                <div>
                  ...
                  <select
                    name="exprience"
                    value={scrapeFormData.exprience}
                    onChange={handleScrapeChange}
                    className="w-full bg-white border border-blue-100 rounded-xl px-2 py-2 text-xs font-bold text-slate-700 outline-none"
                  >
                ...
                <div>
                  ...
                  <input
                    type="number"
                    name="jobPostedDays"
                    value={scrapeFormData.jobPostedDays}
                    onChange={handleScrapeChange}
                    className="w-full bg-white border border-blue-100 rounded-xl px-2 py-2 text-xs font-bold text-slate-700 outline-none"
                  />
                </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-50">
              ...
              <button
                ...
                disabled={
                  isScraping ||
                  (scrapeFormData.platform === "linkedin"
                    ? !scrapeFormData.inputQuery
                    : !scrapeFormData.role)
                }
                className="px-8 py-3.5 bg-blue-600 text-white rounded-2xl font-bold text-sm shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {isScraping ? (
                  <>
            ...
            <div className="space-y-6">
              ...
              <div className="relative group">
                <label className="block text-sm font-bold text-slate-700 mb-3 ml-1">
                  Job Screenshot / Banner
                </label>
                <div
                  className={`border-2 border-dashed rounded-[2rem] p-8 transition-all text-center
                ...
                >
                  ...
                  <div className="flex flex-col items-center">
                    <div
                      className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 text-2xl transition-all
                ...
                {uploadedUrl && (
                  <div className="mt-4 p-3 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center gap-2 text-emerald-700 text-xs font-bold animate-in fade-in slide-in-from-bottom-2">
                    <FiCheckCircle className="shrink-0" /> Job processed and
                    ready!
                  </div>
                )}
              </div>
  ...
  return (
    <div className="flex min-h-screen bg-gray-50/50 font-sans text-slate-900">
      ...
      <main className="flex-1 ml-20 p-4 md:p-8 lg:p-12 transition-all duration-300 overflow-x-hidden">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="max-w-full overflow-hidden">
            ...
            <p className="text-sm md:text-base text-slate-500 font-medium mt-1">
              Explore job postings from various external sources and job boards.
            </p>
          </div>
          {user?.role === "COORDINATOR" && (
            <div className="flex items-center gap-3 shrink-0">
              {/* <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl md:rounded-2xl bg-slate-900 text-white font-bold text-sm hover:bg-slate-800 transition-all shadow-lg shadow-slate-100 cursor-pointer"
              >
                <FiPlus className="text-lg" /> Manual Add
              </button> */}

              <button
                onClick={() => setOpenScrapeModal(true)}
              ...
              >
                {isScraping ? (
                  ...
                  <>
                    <FiLayers className="text-lg" /> Scrape Jobs
                  </>
                )}
              </button>
            </div>
        ...
        {/* Filter Bar */}
        <div className="bg-white rounded-2xl lg:rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-sm mb-10">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[2.5rem] border border-dashed border-slate-200">
              ...
              <p className="text-slate-400 text-sm mt-2 max-w-xs text-center">
                We couldn't find any roles matching your current search
                criteria.
              </p>
              {searchQuery && (
                <button
            ...
            filteredJobs.map((job) => (
              <div
                key={job.id}
                className="group bg-white p-6 md:p-8 rounded-2xl lg:rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 hover:border-blue-100 transition-all duration-300 relative overflow-hidden"
              >
        ...
        {openScrapeModal && renderScrapeModal()}
      </main>
    </div>
  );
};
